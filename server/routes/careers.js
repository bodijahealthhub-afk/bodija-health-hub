...
router.get('/', authenticateToken, requireRole('admin', 'super_admin', 'receptionist'), async (req, res) => {
  try {
    const applications = await db.prepare('SELECT * FROM career_applications ORDER BY created_at DESC').all();
...
router.put('/:id/status', authenticateToken, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    ...
    const result = await db.prepare('UPDATE career_applications SET status = ? WHERE id = ?').run(status, req.params.id);
    if (result.changes === 0) {
...
router.delete('/:id', authenticateToken, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    const result = await db.prepare('DELETE FROM career_applications WHERE id = ?').run(req.params.id);
    if (result.changes === 0) {
      return res.status(404).json({ error: 'Application not found' });
    }
