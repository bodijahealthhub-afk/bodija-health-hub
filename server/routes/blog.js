...
async function handlePublicList(req, res) {
  ...
  const posts = await db.prepare(query).all(...params, parseInt(limit), offset);
...
router.post('/', authenticateToken, requireRole('admin', 'super_admin', 'content_manager'), async (req, res) => {
  try {
    ...
    let finalSlug = slug;
    let counter = 1;
    while (await db.prepare('SELECT id FROM blog_posts WHERE slug = ?').get(finalSlug)) {
...
router.put('/:id', authenticateToken, requireRole('admin', 'super_admin', 'content_manager'), async (req, res) => {
  try {
    ...
    if (title && title !== post.title) {
      ...
      let counter = 1;
      while (await db.prepare('SELECT id FROM blog_posts WHERE slug = ? AND id != ?').get(slug, req.params.id)) {
        slug = `${slug}-${counter}`;
        counter++;
      }
    }
